...
class SwingUpEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.state = (x, x_dot, theta, theta_dot)
               ...
               or x_dot < -self.vel_threshold \
               or theta_dot > self.vel_threshold \
               or theta_dot < -self.vel_threshold
        ...
        if done:
            reward = -np.pi ** 2/(1 - self.discount)
    ...
    def vec_step(self, actions):
        ...
        rewards = -0.5 * (theta ** 2)
        rewards[dones] = -np.pi ** 2/(1-self.discount)
        """
    ...
    def vec_reset(self, num_envs=None):
        ...
        self._states[:, 2] =  np.mod(self._states[:, 2], 2 * np.pi) - np.pi
    ...
    def reset(self):
        ...
        self.state[2] = np.mod(self.state[2], 2 * np.pi) - np.pi
        self.steps_beyond_done = None
