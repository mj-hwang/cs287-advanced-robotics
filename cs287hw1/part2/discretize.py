...
class Discretize(DiscretizeWrapper):
    ...
    def get_discrete_state_from_cont_state(self, cont_state):
        ...
        if self.mode == 'nn':
            ...
            # ix = upper_i[0]
            # iy = upper_i[1]
            # indices = np.array([[ix-1, iy-1], 
            #                     [ix-1, iy],
            #                     [ix, iy-1],
            #                     [ix, iy]])
            # states = self.get_id_from_coordinates(indices)
            # x = (cont_state[0][0] - self.state_points[0][ix-1]) / (self.state_points[0][ix] - self.state_points[0][ix-1])
            # y = (cont_state[1][0] - self.state_points[1][iy-1]) / (self.state_points[1][iy] - self.state_points[1][iy-1])
            # probs = np.array([(1-x) * (1-y),
            #                   x * (1-y),
            #                   (1-x) * y,
            #                   x * y])
            # print(states)
